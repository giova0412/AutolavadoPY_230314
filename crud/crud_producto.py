...
def get_producto_by_id(db: Session, producto_id: int):
    return db.query(models.model_producto.Producto).filter(models.model_producto.Producto.Id == producto_id).first()

def get_producto_by_nombre(db: Session, nombre: str):
    return db.query(models.model_producto.Producto).filter(models.model_producto.Producto.nombre == nombre).first()
